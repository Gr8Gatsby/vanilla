      ...
      return(
          <div className="dataTable">
              ...
              {/* {rows.map(function(cells, index){
                return <DataRow key={ index } cells={ cells }></DataRow>
              })} */
                records.map(row => {
...
class DataRow extends React.Component{
  render(){
    ...
    return(
      <div>
        {Object.values(this.props.row._props).forEach(attr => {
          cells.push(attr);
        })}
        {cells.map(value => {
          return <DataCell label="Name" value={ value }></DataCell>
        })}
      </div>
    )
  }
